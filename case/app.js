const LIBRARY_CONFIG = {
  ...
  "subtitle": "案例洞察档案",
  "kicker": "Case Notes",
  "description": "把案例、赛道、反常识点和强度评级做成可快速翻阅的知识卡片。",
  "csvFile": "案例库.csv",
  "styleName": "Notion",
  "globalName": "__CASE_LIBRARY_DATA__",
  "pageSize": 100,
  "uncategorized": "未分类",
  "titleField": "案例名称",
  "dateField": "案例日期",
  ...
  "theme": {
    ...
    "muted": "#6b625c",
    "line": "#e8e2d9",
    "shadow": "0 10px 28px rgba(35, 31, 28, 0.08)"
  }
};
...
function buildMergedSentence(item) {
  ...
  if (LIBRARY_CONFIG.slug === "case") {
    return [
      ...
      `来源线索为${summarizeSource(item["案例来源(文件地址/文章链接)"])}`,
    ].join("，") + "。";
  }
...
function summarizeSource(value) {
  const text = cleanText(value);
  if (!text) return "未填写";
  if (/^https?:\/\//i.test(text)) return "线上原文链接";
  if (/^[a-z]:[\\/]/i.test(text) || /[\\/]/.test(text) || /\.(md|json|csv|xlsx?)$/i.test(text)) {
    return "本地归档文件";
  }
  return text;
}
